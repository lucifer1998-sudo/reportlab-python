...
def create_vertical_bar(value, min_value, max_value, width=25, height=100, label_color=colors.royalblue):
    ...
    d.add(Rect(0, 0, width, height, fillColor=colors.whitesmoke, strokeColor=None))

    for i in range(int(min_value), int(max_value) + 1):
        y = (i - min_value) / (max_value - min_value) * height
        d.add(Line(0, y, width, y, strokeColor=colors.lightgrey, strokeWidth=0.5))
    ...
    for row_idx, row in enumerate(chart['rows']):
        ...
        for col_idx, cell in enumerate(row['cols']):
            ...
            if cell.get('type') == 'bar':
                ...
                # print("col index = " + str(col_idx) + ", row index = " + str(row_idx) + ", text = " + str(text))
                # if(row_idx < 2 and ( chart['cols'] == 7 and col_idx == 0 )) : 
                if( (chart['cols'] == 7 and col_idx == 0) or ( chart['cols'] == 8 and col_idx < 2 ) ) : 
